...
export function AdminLawyersDetailSheet(props: {
	...
	const activeProfileId = props.profileId;
	...
	const deals = [...detail.deals.active, ...detail.deals.recent];
	const dealById = new Map(deals.map((deal) => [deal._id, deal]));
	const representationDealId = detail.actionTargets.representationDealId;
	const replacementDealId = detail.actionTargets.replacementDealId;
	const primaryDeal =
		(representationDealId ? dealById.get(representationDealId) : undefined) ??
		(replacementDealId ? dealById.get(replacementDealId) : undefined) ??
		null;
	...
	function submitEvidence() {
		...
		if (!(representationDealId && evidenceMode)) {
			setError("A deal is required for this evidence action.");
			return;
		}
		const payload = {
			attachmentIds: [],
			dealId: representationDealId,
			evidenceNote,
			reason:
				evidenceMode === "verify"
