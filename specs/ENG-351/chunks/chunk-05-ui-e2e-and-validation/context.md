# Chunk Context: ui e2e and validation
...
Primary code areas: `src/components/listings/ListingDetailPage.tsx`, `src/components/listings/MarketplaceListingDetailPage.tsx`, `src/routes/listings/$listingId.tsx`, `src/routes/listings/route.tsx`, `src/test/listings/listing-detail-checkout.test.tsx`, `src/test/routes/listings-route.test.tsx`, `e2e/marketplace/**`, and `playwright.config.ts`. Preserve the authenticated layout pattern using `Authenticated` / `AuthLoading` for suspense query route trees.
