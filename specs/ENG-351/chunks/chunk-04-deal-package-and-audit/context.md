# Chunk Context: deal package and audit
...
Primary code areas: `convex/checkout/dealHandoff.ts`, `convex/deals/mutations.ts`, `convex/deals/queries.ts`, `convex/engine/effects/dealClosing.ts`, `convex/engine/effects/dealClosingEffects.ts`, `convex/documents/dealPackages.ts`. Invariants: create exactly one deal for a completed paid checkout, do not create a deal from expired/abandoned/refunded sessions, do not call `reserveShares` again for checkout-owned reservations, package signatories come from checkout participants.
