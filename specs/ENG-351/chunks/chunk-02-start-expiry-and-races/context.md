# Chunk Context: start expiry and races
...
Primary code areas: `convex/checkout/actions.ts`, `convex/checkout/mutations.ts`, `convex/ledger/mutations.ts`, `convex/listings/marketplace.ts`, `convex/listings/portalVisibility.ts`, `convex/listings/marketplaceShared.ts`. Invariants: server re-checks auth, portal, listing status, lender constraints, requested fractions, lawyer payload, and availability; availability is ledger-derived; provider cleanup never controls inventory truth.
