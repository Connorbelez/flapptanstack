...
## Active focus
- Close out the post-audit verification pass with the duplicate-host and cache-boundary fixes landed, while keeping the remaining repo-level blockers explicit.
...
- Fresh reruns on 2026-04-20 reconfirmed `bunx convex codegen`, `bun typecheck`, and the focused Vitest suite pass. `bun check` and CodeRabbit remain blocked for the same external reasons already captured above.
