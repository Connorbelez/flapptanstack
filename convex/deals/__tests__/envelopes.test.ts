...
import { verifyDocumensoSignature } from "../../payments/webhooks/verification";
...
import { normalizeDocumensoEventType } from "../envelopeWebhooks";
...
describe("deal envelope helpers", () => {
	...
	it("normalizes Documenso event names and verifies HMAC signatures", () => {
		...
		const body = JSON.stringify({ event: "document.completed" });
		const signature = createHmac("sha256", "secret").update(body).digest("hex");
		expect(verifyDocumensoSignature(body, signature, "secret")).toBe(true);
		expect(verifyDocumensoSignature(body, signature, "wrong")).toBe(false);
		expect(
			verifyDocumensoSignature(body, "not-a-valid-signature", "secret")
		).toBe(false);
	});
});
	...
	it("reissues attempts with supersede lineage while preserving history", async () => {
		...
		const second = await t
			...
			.mutation(api.deals.envelopes.reissueEnvelopeAttempt, {
				...
				provider: "documenso",
				providerDocumentId: "doc_new",
				recipientTokens: [
					{
						platformRole: "lender_primary",
						providerRecipientId: "rec_buyer_2",
					},
					{
						platformRole: "lawyer_primary",
						providerRecipientId: "rec_lawyer_2",
					},
				],
			});
...
describe("deal envelope webhooks", () => {
	...
	it("returns completion emission only after all active required recipients are complete", async () => {
		const { t, dealId, instanceId } = await seedEnvelopeFixture();
