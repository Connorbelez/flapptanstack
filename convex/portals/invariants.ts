...
async function assertNoCrossHostCollision(
	ctx: PortalRegistryCtx,
	candidate: NormalizedPortalRegistryCandidate,
	currentPortalId?: Id<"portals">
) {
	if (candidate.productionHost) {
		const rows = await ctx.db
			.query("portals")
			.withIndex("by_local_host", (query) =>
				query.eq("localHost", candidate.productionHost)
			)
			.take(2);
		const conflict = rows.find((portal) => portal._id !== currentPortalId);
		if (conflict) {
			throw new ConvexError(
				`Production host "${candidate.productionHost}" collides with another portal's local host (${String(conflict._id)})`
			);
		}
	}
	...
	if (candidate.localHost) {
		const rows = await ctx.db
			...
			.withIndex("by_production_host", (query) =>
				query.eq("productionHost", candidate.localHost)
			)
			.take(2);
		const conflict = rows.find((portal) => portal._id !== currentPortalId);
		if (conflict) {
			throw new ConvexError(
				`Local host "${candidate.localHost}" collides with another portal's production host (${String(conflict._id)})`
			);
		}
	}
}
...
export async function assertPortalRegistryInvariants(
	...
	await assertNoCrossHostCollision(
		ctx,
		normalizedCandidate,
		normalizedCandidate.currentPortalId
	);
